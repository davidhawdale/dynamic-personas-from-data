...
1. Read all files in `.claude/rules/` — these contain operational standards that Claude Code auto-loads but Gemini must read explicitly.
...
3. Workflow shortcuts live in `.claude/skills/` — when the user invokes a workflow (e.g., "run the research question analysis"), read the relevant `SKILL.md` file and follow the procedure inline. There is no slash-command support in Gemini.
4. Agent definitions in `.claude/agents/` are prompt modules — read the agent `.md` file and execute the agent's instructions inline. There is no isolated context in Gemini; the agent runs in your main context.
5. Call hierarchy equivalent: User → Procedure (SKILL.md) → Inline execution (agent .md) → Python script.

**Model tier equivalents for Gemini:**

| This project | Gemini equivalent |
|---|---|
| Haiku (mechanical) | `gemini-2.0-flash` |
| Sonnet (judgement) | `gemini-2.0-pro` |
| Opus (planning) | `gemini-2.0-ultra` |
