...
- Sequence:
  1. Run `python3 02-workflows/build-dynamic-personas/prepare-roleplay-pack.py`.
  2. Run `python3 02-workflows/build-dynamic-personas/verify-roleplay-pack.py`.
  3. Optional smoke test: `python3 02-workflows/build-dynamic-personas/run-roleplay-session.py --question "What should the first MVP focus on?"`
  4. If smoke output exists, run `python3 02-workflows/build-dynamic-personas/verify-roleplay-response.py --file <smoke-output-path>`.
...
- Sequence:
  1. Verify Phase 7 assets: `python3 02-workflows/build-dynamic-personas/verify-roleplay-pack.py`
  2. Start app: `python3 02-workflows/build-dynamic-personas/run-roleplay-app.py`
  3. Optional API smoke test:
     - Create session: `POST /api/session`
     - Ask one question: `POST /api/session/{session_id}/ask`
  4. If smoke output is captured to file, run `python3 02-workflows/build-dynamic-personas/verify-roleplay-response.py --file <response-file>`
...
- **If validation passes:** "Phase 8 passed. Please review app outputs and session artifacts. Ready to continue?"
- **If validation fails:** "Phase 8 has validation failures. Would you like to re-run with correction instructions?"

Do not proceed to any next phase until the user explicitly says yes.
